...
# 导入数据下载模块
from data_downloader import download_yahoo_finance_data, get_popular_symbols, list_available_intervals
...
def load_data():
    ...
    try:
        ...
        data_info = {
            ...
            # 返回时间戳数组，用于前端显示准确的日期
            'timestamps': [ts.isoformat() for ts in df['timestamps']] if 'timestamps' in df.columns else []
        }
    ...
    try:
        ...
        if MODEL_AVAILABLE and predictor is not None:
            try:
                ...
                # Process time period selection - 使用 end_index 从后往前计算
                end_index = data.get('end_index')
                start_date = data.get('start_date')
                
                print(f"[DEBUG] Received parameters: end_index={end_index}")
                
                if end_index is not None:
                    # 使用结束行索引，从后往前计算起始位置
                    try:
                        end_idx = int(end_index)
                        window_size = lookback + pred_len  # 520
                        
                        # 从 end_idx 往前推算 start_idx
                        # end_idx 是窗口的最后一行（包含），所以 start_idx = end_idx - window_size + 1
                        start_idx = end_idx - window_size + 1
                        
                        # 确保 start_idx 不小于 0
                        if start_idx < 0:
                            return jsonify({'error': f'Insufficient data before row {end_idx}, need {window_size} data points, only {end_idx + 1} available'}), 400
                        
                        # 确保 end_idx 不超出范围
                        if end_idx >= len(df):
                            return jsonify({'error': f'End row {end_idx} exceeds data range (max: {len(df) - 1})'}), 400
                        
                        # Use data from start_idx to end_idx
                        x_df = df.iloc[start_idx:start_idx+lookback][required_cols]
                        x_timestamp = df.iloc[start_idx:start_idx+lookback]['timestamps']
                        y_timestamp = df.iloc[start_idx+lookback:end_idx+1]['timestamps']
                        
                        # Calculate actual time period length
                        start_timestamp = df['timestamps'].iloc[start_idx]
                        end_timestamp = df['timestamps'].iloc[end_idx]
                        time_span = end_timestamp - start_timestamp
                        
                        # 打印日志：输入数据时间范围
                        input_start = x_timestamp.iloc[0]
                        input_end = x_timestamp.iloc[-1]
                        pred_start = y_timestamp.iloc[0]
                        pred_end = y_timestamp.iloc[-1]
                        print(f"========== Prediction Info (By End Index) ==========")
                        print(f"Received end_index from frontend: {end_index}")
                        print(f"Calculated start_idx: {start_idx}, end_idx: {end_idx}")
                        print(f"Data range: row {start_idx} to row {end_idx}")
                        print(f"Input data (lookback={lookback}): row {start_idx} to row {start_idx+lookback-1}")
                        print(f"  Start: {input_start}")
                        print(f"  End:   {input_end}")
                        print(f"Prediction target (pred_len={pred_len}): row {start_idx+lookback} to row {end_idx}")
                        print(f"  Start: {pred_start}")
                        print(f"  End:   {pred_end}")
                        print(f"====================================================")
                        
                        prediction_type = f"Kronos model prediction (row {start_idx} to {end_idx}, time span: {time_span})"
                        
                        # 标记使用了索引模式
                        used_index_mode = True
                    except Exception as e:
                        print(f"Warning: Failed to use end_index '{end_index}', falling back. Error: {e}")
                        end_index = None
                        used_index_mode = False
                
                elif start_date and start_date.strip():
                    try:
                        # Custom time period - fix logic: use data within selected window
                        start_dt = pd.to_datetime(start_date).tz_localize(None)  # 移除时区信息
                        
                        # 确保 df['timestamps'] 也没有时区信息
                        ...
                        pred_start = y_timestamp.iloc[0]
                        pred_end = y_timestamp.iloc[-1]
                        print(f"========== Prediction Info (By Date) ==========")
                        print(f"Input data (lookback={lookback}):")
                        print(f"  Start: {input_start}")
                        print(f"  End:   {input_end}")
                        print(f"Prediction target (pred_len={pred_len}):")
                        print(f"  Start: {pred_start}")
                        print(f"  End:   {pred_end}")
                        print(f"================================================")
                        ...
                        # If date parsing fails, fall back to using latest data
                        print(f"Warning: Failed to parse start_date '{start_date}', using latest data. Error: {e}")
                        start_date = None
                        used_index_mode = False
                
                if end_index is None and (not start_date or not start_date.strip()):
                    # Use latest data
                    x_df = df.iloc[:lookback][required_cols]
                    x_timestamp = df.iloc[:lookback]['timestamps']
                    y_timestamp = df.iloc[lookback:lookback+pred_len]['timestamps']
                    
                    # 打印日志：输入数据时间范围（使用最新数据）
                    input_start = x_timestamp.iloc[0]
                    input_end = x_timestamp.iloc[-1]
                    pred_start = y_timestamp.iloc[0] if len(y_timestamp) > 0 else 'N/A'
                    pred_end = y_timestamp.iloc[-1] if len(y_timestamp) > 0 else 'N/A'
                    print(f"========== Prediction Info (Latest Data) ==========")
                    print(f"Input data (lookback={lookback}):")
                    print(f"  Start: {input_start}")
                    print(f"  End:   {input_end}")
                    print(f"Prediction target (pred_len={pred_len}):")
        ...
        # 根据使用的模式获取实际数据 - 使用 start_idx（已在上面计算好）
        if end_index is not None:
            # 使用 end_index 模式，start_idx 已经计算好
            try:
                if start_idx + lookback + pred_len <= len(df):
                    actual_df = df.iloc[start_idx+lookback:start_idx+lookback+pred_len]
                ...
                print(f"Warning: Failed to get actual data by end_index: {e}")
        
        elif start_date and start_date.strip():  # Custom time period
            try:
                ...
                print(f"Warning: Failed to process actual data for custom time period: {e}")
        ...
        if end_index is not None:
            historical_start_idx = start_idx
        elif start_date and start_date.strip():
            try:
        ...
        if 'timestamps' in df.columns:
            if end_index is not None:
                # 使用 end_index 模式计算时间戳
                try:
                    last_timestamp = df['timestamps'].iloc[start_idx + lookback - 1]
                ...
                try:
                    ...
                    # Fall back to latest data timestamps
                    last_timestamp = df['timestamps'].iloc[-1]
                    time_diff = df['timestamps'].iloc[1] - df['timestamps'].iloc[0]
                    future_timestamps = pd.date_range(
                        start=last_timestamp + time_diff,
        ...
        try:
            save_prediction_results(
                ...
                prediction_params={
                    ...
                    'top_p': top_p,
                    'sample_count': sample_count,
                    'end_index': end_index if end_index is not None else None,
                    'start_idx': start_idx if 'start_idx' in locals() else None,
...
def download_data():
    ...
    try:
        ...
        return jsonify({'error': f'下载数据失败: {str(e)}'}), 500

@app.route('/api/popular-symbols')
def api_get_popular_symbols():
    """获取常用股票代码列表"""
    return jsonify(get_popular_symbols())

@app.route('/api/available-intervals')
def api_get_available_intervals():
    """获取可用的数据间隔"""
    return jsonify(list_available_intervals())
